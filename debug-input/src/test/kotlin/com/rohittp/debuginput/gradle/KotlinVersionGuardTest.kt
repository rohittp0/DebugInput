...
class KotlinVersionGuardTest {
    ...
    fun `the supported minor passes`() {
        assertNull(kotlinVersionMismatchMessage("1.0.0", "2.4.20"))
        assertNull(kotlinVersionMismatchMessage("1.0.0", "2.4.0"))
        assertNull(kotlinVersionMismatchMessage("1.0.0", "2.4.30-Beta2"))
    }

    @Test
    fun `another minor fails`() {
        listOf("2.3.21", "2.5.0", "3.0.0", "1.9.24").forEach { version ->
            assertTrue(
    ...
    fun `the message names both versions and what to do`() {
        val message = kotlinVersionMismatchMessage("1.0.0", "2.3.21")
        ...
        assertTrue("2.4" in message, message)
        assertTrue("2.3.21" in message, message)
